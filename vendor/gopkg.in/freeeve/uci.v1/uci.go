...
func (eng *Engine) Perft(depth int) (total int, moves map[string]int, err error) {
	...
	_, err = eng.stdin.WriteString(fmt.Sprintf("go perft %d\n", depth))
	if err != nil {
		return
	}
	err = eng.stdin.Flush()
	...
	for {
		...
		if strings.HasPrefix(line, "Nodes searched:") {
			...
			total, err := strconv.Atoi(parts[len(parts)-1])
			if err != nil {
				return 0, nil, err
			}
			return total, moves, nil
		}
		matches := perftMoveRegex.FindStringSubmatch(line)
		if len(matches) == 3 {
			moveCount, err := strconv.Atoi(matches[2])
			if err != nil {
				return 0, nil, err
			}
			moves[matches[1]] = moveCount
		}
	}
